...
contract MoodNftTest is Test {
    MoodNft nft_contract;
    DeployMoodNft public deployer;
    address USER = makeAddr("user"); // Create a user address for testing
    ...
    function testViewTokenUri() public {
        ...
        // console.log("Token URI for token ID 1: %s", nft_contract.tokenURI(1));
        assertEq(nft_contract.tokenURI(1), expectedTokenUri(HAPPY_SVG_URI));
        vm.stopPrank();
    }
    ...
    function testFlipTokenToSad() public {
        vm.startPrank(USER);
        nft_contract.mintNft();
        nft_contract.flipMood(1); // Flip to SAD mood
        assertEq(nft_contract.tokenURI(1), expectedTokenUri(SAD_SVG_URI));
        vm.stopPrank();
    }
}
